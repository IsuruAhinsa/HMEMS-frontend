...
const WardPrList = () => {
  ...
  useEffect(() => {
    const filtered = ward
      ...
      .sort((a, b) => {
        if (a.prType === "Emergency" && b.prType !== "Emergency") return -1;
        if (a.prType !== "Emergency" && b.prType === "Emergency") return 1;
        if (a.prType === "Stand" && b.prType !== "Stand") return -1;
        if (a.prType !== "Stand" && b.prType === "Stand") return 1;
        if (a.prType === "Minor" && b.prType !== "Minor") return -1;
        if (a.prType !== "Minor" && b.prType === "Minor") return 1;
        if (a.prType === "Other" && b.prType !== "Other") return -1;
        if (a.prType !== "Other" && b.prType === "Other") return 1;
        return 0;
      });
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentItems.map((wardItem) => (
                    <tr
                      key={wardItem._id}
                      className={`${
                        wardItem.prType === "Emergency"
                          ? "bg-red-200"
                          : wardItem.prType === "Stand"
                          ? "bg-yellow-200"
                          : wardItem.prType === "Minor"
                          ? "bg-green-200"
                          : wardItem.prType === "Other"
                          ? "bg-blue-200"
                          : ""
                      }`}
                    >
                        ...
                        {canNotCreateUser && (
                          <>
                              ...
                              <button
                                ...
                                onClick={() => handleDelete(wardItem._id)} // Trigger the delete on click
                              >
                                Reject
                              </button>
