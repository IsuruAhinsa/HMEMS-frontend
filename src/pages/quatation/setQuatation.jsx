...
const SetQuotation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const vendorQuotation = {
      ...
      arrivalTimePeriod,
      warrantyPeriod,
      numberOfUnits,  // Ensure it's an integer
      comments,
      role:user.role,
      name:user.firstName,
      genericName,
    ...
    const fetchQuotation = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setrequestpriority(json.requestpriority);
          setNumberOfUnits(json.numberOfUnits);
          setWard(json.ward);
        ...
        <CardContent className="grid gap-4">
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                <Label htmlFor="currentBrand">Current Available Brand</Label>
                <Input
                  type="text"
                  id="currentBrand"
                  value={currentBrand}
            ...
            <div>
                <Label htmlFor="setrequestpriority">Priority</Label>
                <Input
                  type="text"
                  readOnly
                  id="Priority"
                  value={requestpriority}
                  onChange={(e) =>  setrequestpriority(e.target.requestpriority)}
                />
              </div>
              
              <div>
                <Label htmlFor="setrequestpriority">Ward</Label>
                <Input
                  type="text"
                  readOnly
                  id="Priority"
                  value={ward}
                  onChange={(e) =>  setWard(e.target.requestpriority)}
                />
              </div>

              <div>
                <Label htmlFor="setrequestpriority"> Room Number</Label>
                <Input
                  type="text"
                  readOnly
                  id="Priority"
                  value={roomNumber}
                  onChange={(e) =>  setRoomNumber(e.target.roomNumber)}
                />
              </div>
              
              <div>
                <Label htmlFor="setrequestpriority"> Ward Line Matrix</Label>
                <Input
                  type="text"
                  readOnly
                  id="Priority"
                  value={wardLineMatrix}
                  onChange={(e) =>  setWardLineMatrix(e.target.wardLineMatrix)}
                />
              </div>
